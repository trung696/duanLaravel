...
class CategoryNews extends Component
{
    ...
    public function create()
    {
        $this->validate();
        if($this->file_name){
            $this->file_name = $this->image->store('/category-new', 'public');
        }

        CategoryNew::create($this->modelData());
        $this->reset();
    ...
    public function update()
    {
        ...
        CategoryNew::find($this->modelId)->update($this->modelData());
        $this->modalFormVisible = false;
    ...
    {
        CategoryNew::destroy($this->modelId);
    ...
    {
        $data = CategoryNew::find($this->modelId);
        $this->name = $data->name;
...
//    Validate
    ...
    {
        return [
            'name' => 'required|unique:App\Models\CategoryNew,name',
            'slug' => 'required|unique:App\Models\CategoryNew,slug',
        ];
    }
    ...
    {
        return CategoryNew::where('name','like', '%' . $this->search . '%')->paginate(8);
