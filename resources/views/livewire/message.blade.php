                        ...
                        <div class="card-body chatbox p-0">
                            <ul class="list-group list-group-flush" wire:poll="render">
                                @foreach($users as $user)
                                    @php
                                        $not_seen = \App\Models\ChatMessage::where('user_id', $user->id)->where('receiver', auth()->id())->where('is_seen', false)->get() ?? null
                                    @endphp
    ...
    {{--                                    @else--}}
                            ...
                            @else
                                No messages to show
                            @endif
                            @if(!isset($clicked_user) and auth()->user()->id_role == 1)
